...
const TOKEN_TTL_SECONDS     = 300;
const NONCE_TTL_SECONDS     = 120;   // nonce expires if not used within 2 minutes
const MIN_ELAPSED_MS        = 3000;  // client must have spent ≥3s on page
const PROOF_TS_TOLERANCE_S  = 30;    // proof timestamp must be within 30s of server time
...
async function handleVerify(context) {
  ...
  // ── Issue stream token + session key ───────────────────────────────
  const expiry    = Math.floor(Date.now() / 1000) + TOKEN_TTL_SECONDS;
  const payload   = `${ip}:${expiry}`;
  const signature = await hmacSign(env.STREAM_HMAC_SECRET, payload);
  const streamToken = btoa(`${payload}:${signature}`);
...
async function validateBehaviourProof(proof, requestIp, env) {
  ...
  // 2. Delete immediately — one-time use, prevents replay attacks
  await env.NONCE_STORE.delete(`nonce:${nonce}`);
  ...
  // 4. Verify HMAC(nonce, payload)
  const expectedSig = await hmacSign(nonce, payload);
  if (!timingSafeEqual(sig, expectedSig)) {
    return { valid: false, reason: 'Invalid behaviour proof signature' };
  }

  // 5. Parse and validate payload fields
  // Format: "moved:1|scrolled:1|clicked:1|elapsed:NNNN|ts:TTTT"
  const fields = {};
  for (const part of payload.split('|')) {
  ...
  if (fields.moved    !== '1') return { valid: false, reason: 'Insufficient mouse movement' };
  if (fields.scrolled !== '1') return { valid: false, reason: 'No scroll detected' };
  if (fields.clicked  !== '1') return { valid: false, reason: 'No click detected' };

  const elapsed = parseInt(fields.elapsed ?? '0');
  if (isNaN(elapsed) || elapsed < MIN_ELAPSED_MS) {
    return { valid: false, reason: 'Insufficient time on page' };
  }

  // 6. Proof timestamp freshness
  const proofTs = parseInt(fields.ts ?? '0');
  const now     = Math.floor(Date.now() / 1000);
  if (isNaN(proofTs) || Math.abs(now - proofTs) > PROOF_TS_TOLERANCE_S) {
    return { valid: false, reason: 'Behaviour proof timestamp out of range' };
